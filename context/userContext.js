...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    ...
    const res = await axios.post("https://goofy-purple-lungfish.glitch.me/api/v1/login", userData);
  ...
  const logout = async () => {
    const res = await axios.get("https://goofy-purple-lungfish.glitch.me/api/v1/logout");
    setUser(null);
    setUserToken(null);
  ...
  const register = async (name, email, password, role, photo) => {
    ...
    const res = await axios.post("https://goofy-purple-lungfish.glitch.me/api/v1/signup", formData);
    const data = await res.data;
    setUser(data.user);
    setUserToken(data.token);
    return data;
